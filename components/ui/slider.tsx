...
function Slider({ className, defaultValue, value, min = 0, max = 100, ...props }: SliderPrimitive.Root.Props) {
  ...
  return (
    <SliderPrimitive.Root className={cn('data-horizontal:w-full data-vertical:h-full', className)} data-slot="slider" defaultValue={defaultValue} value={value} min={min} max={max} thumbAlignment="center" {...props}>
      <SliderPrimitive.Control className="relative flex w-full touch-none items-center select-none data-disabled:opacity-50 data-vertical:h-full data-vertical:min-h-40 data-vertical:w-auto data-vertical:flex-col">
        <SliderPrimitive.Track data-slot="slider-track" className="relative grow overflow-hidden rounded-md bg-gray-200 select-none data-horizontal:h-1 data-horizontal:w-full data-vertical:h-full data-vertical:w-1.5">
          <SliderPrimitive.Indicator data-slot="slider-range" className="bg-[#52B788] select-none data-horizontal:h-full data-vertical:w-full" />
        </SliderPrimitive.Track>
        {Array.from({ length: _values.length }, (_, index) => (
          <SliderPrimitive.Thumb data-slot="slider-thumb" key={index} className="outline-none select-none">
            <div
              className=" relative flex items-center justify-center size-8 shrink-0 text-[12px] font-medium text-[#2D3748] rounded-full border-2 border-gray-300 bg-white transition-all select-none -translate-y-7 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 after:content-[''] after:absolute after:-bottom-2 after:left-1/2 after:-translate-x-1/2 after:border-x-4
            after:border-x-transparent after:border-t-4 after:border-t-gray-300">
              You
            </div>

            <div className="absolute top-6 left-1.5 text-[11px] font-semibold text-[#2B3674] whitespace-nowrap">{_values[index]}%</div>
